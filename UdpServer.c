...
int post(){
    ...
    char msg_dir_file[50] = "";
    char nmsg[4] ="";
    char aux[7] = "";
    char *ptr = NULL;
    FILE *f;
    ssize_t nbytes,nleft,nwritten,nread,dread;
    ...
    sprintf(msg_dir,"GROUPS/%s/MSG/%s",gid,nmsg);
    mkdir(msg_dir,0700);
    sprintf(msg_dir_author,"GROUPS/%s/MSG/%s/A U T H O R.txt",gid,nmsg);
    sprintf(msg_dir_text,"GROUPS/%s/MSG/%s/T E X T.txt",gid,nmsg);

    f = fopen(msg_dir_author,"w");
    fwrite(aux,1,strlen(aux),f);
    fclose(f);
    ...
    printf("fnameeee %s",fname);
    sprintf(msg_dir_file,"%s/%s",msg_dir,fname);
    printf("dir %s",msg_dir_file);
    f = fopen(msg_dir_author,"wb");
    ...
    if(strcmp(fname,"")!=0)
        ...
        while(nleft > 0){
            ...
            printf("ptr %c",ptr[0]);
            printf("nleft %ld",nleft);
            printf("nlread %ld",nread);
            if(ptr[0] == ' '){break; printf("sai");}
            nleft-=nread;
            ptr+=nread;
        }   
        printf("fsize %s",fsize);
        while(nread < atoi(fsize)){
            strcpy(data,"");
            nread += read(newfd,ptr,1);
            nleft = 512;
            ptr = data;
            ...
            fwrite(data,sizeof(unsigned char),512,f);
        }
        fclose(f);
}
int TcpCommands(){
